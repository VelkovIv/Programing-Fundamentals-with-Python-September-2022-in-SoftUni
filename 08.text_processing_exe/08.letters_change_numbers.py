...
current_input = [element.strip() for element in input().split()]

total_sum = 0

for element in current_input:
    first_character = element[0:1]
    last_character = element[len(element) - 1:len(element)]
    number = int(element[1:len(element) - 1])

    if first_character.isupper():
        total_sum += number / (ord(first_character) - 64)
    elif first_character.islower():
        total_sum += number * (ord(first_character) - 96)

    if last_character.isupper():
        total_sum -= (ord(last_character) - 64)
    elif last_character.islower():
        total_sum += (ord(last_character) - 96)
